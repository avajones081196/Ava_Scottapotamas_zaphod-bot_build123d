"""
zaphod_bot_mec_fbcoup_5x15mm_presentation_post_build123d.py

Reference STL: https://github.com/Scottapotamas/zaphod-bot/blob/master/mechanical/fibre_couplers/manf_outputs/5x15mm_presentation_post.STL
...
Sketch summary (planes auto-detected; both axis-aligned to Z = 4.582):
  ...
  S1  Z = 4.582  8 Lines + 4 three-point arcs = 12 primitives forming a
                  ...
                  not an open chain.
  S2  Z = 4.582  1 Line from (4.397, 20.0, 4.582) to (4.397, 16.3, 4.582)
...
Code-style reference: zaphod_bot_mec_fbcoup_5x15mm_2mm_globe_post_build123d.py
(used for helpers and patterns only — geometry rules differ here:
plane is Z=4.582 instead of X=3.847; profile is CLOSED instead of open
so the chain walker is the closed-loop variant; counts are 8+4=12
primitives instead of 10+5=15; no Step-1/Step-15 free-endpoint
ownership check because there are no free endpoints).
"""
import csv
import math
import re
...
#       profile wire.
#
# Unlike the 2mm-globe-post variant, this profile is fully enclosed: every
# endpoint has exactly two neighbours, so the chain walker is the
# closed-loop variant and there is no Step-1/Step-N free-endpoint
# ownership check.
# ══════════════════════════════════════════════════════════════════════════
