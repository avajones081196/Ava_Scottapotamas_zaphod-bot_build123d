...
The main script reads the cleaned CSVs and rebuilds the geometry as a series of numbered guidelines (G1, G2, G3, …). The guideline count scales with part complexity — for the 3mm_globe, 5x15mm_1mm_globe_post and 5x15mm_2mm_globe_post G1–G4 is enough (G3 is always the final export step). For larger parts the count will grow into the dozens, following the same pattern proven on the PICSY companion repo (where individual parts run up to G92). The pattern is:
...
- **Surface-revolve / extrude / loft** — the part-specific geometric operation. For the three globe-family parts (`3mm_globe`, `5x15mm_1mm_globe_post`, `5x15mm_2mm_globe_post`) this is a 360° revolve of an open profile wire about a construction-axis line. For future parts it may include any of the extrude / loft / boolean / trim patterns documented in the PICSY companion repo.
...
The **cross-section slice comparison** has proven especially valuable on complex parts: it cuts both meshes at multiple Y-plane positions (auto-derived from mesh bounds) and compares the outline perimeter at each cut. Aggregate metrics like Hausdorff or mean-distance can mask a 1–2% feature-level discrepancy because the affected region is small compared to the total surface — the slice test bypasses this by comparing geometry at specific cross-sections. On both globe-post variants every slice passed comfortably: ≤ 0.24% perimeter error on the 1mm-post, ≤ 0.283% on the 2mm-post, with the worst slice in each part landing at the narrow neck just before the globe terminates (where a small absolute deviation resolves to a relatively larger percentage of a small perimeter).
