...
The main script reads the cleaned CSVs and rebuilds the geometry as a series of numbered guidelines (G1, G2, G3, …). The guideline count scales with part complexity — for the 3mm_globe, 5x15mm_1mm_globe_post, 5x15mm_2mm_globe_post, 5x15mm_presentation_post and 5x25mm_presentation_post G1–G4 is enough (G3 is always the final export step). For larger parts the count will grow into the dozens, following the same pattern proven on the PICSY companion repo (where individual parts run up to G92). The pattern is:
...
- **Surface-revolve / extrude / loft** — the part-specific geometric operation. For the five fibre-coupler parts built so far (`3mm_globe`, `5x15mm_1mm_globe_post`, `5x15mm_2mm_globe_post`, `5x15mm_presentation_post`, `5x25mm_presentation_post`) this is a 360° revolve of a profile wire about a construction-axis line. For future parts it may include any of the extrude / loft / boolean / trim patterns documented in the PICSY companion repo.
...
- **~15 min** deciding what to *remove* from the 1mm-post script before running. The 1mm-post script carries three pieces of machinery that this part does not need: (a) the spline parser (`parse_splines`), (b) the `_row_all_xyz_triples` overflow handler for spline rows with > 3 control points, and (c) the entire bridge-insertion block — peek-at-S2 axis discovery, off-axis candidate filter, `BRIDGE_V_TOL` constant, trial-walker selection, `_find_free_endpoints` helper. All of that is dead code for a clean 15-primitive open chain, and leaving it in would have made the file ~120 lines longer for no benefit. Removed everything not used. The 2mm-post script ends up shorter than the 1mm-post script despite covering one more primitive — which is the right outcome when the source CSV is cleaner.
- **~10 min** on the first dry run. CSV freshness banner printed the two CSVs at 907 B and 97 B. Plane detector landed on `X = 3.847` for both sketches as expected. Parser counted 10 lines + 5 arcs in S1 and 1 line in S2 — passed the `EXPECTED_*` guard. Walker found exactly 2 free endpoints at `(Y=17.0, Z=4.008)` from Step 15's line and `(Y=21.916, Z=4.008)` from Step 1's arc — both on the revolve axis at Z=4.008, just as the on-axis-closure invariant requires. No bridge needed, no precision-leak path exercised, chain walked all 15 edges first try.
- **~20 min** on G4 and the comparison run. The OCCT `BRepPrimAPI_MakeRevol(wire, …)` path produced 15 faces in one shot at total area 414.11 mm² — close to the reference STL's 413.49 mm². `.clean()` ran without warning, the STL+STEP exports landed without issue, and the comparison script reported every aggregate metric in the 🟢 EXCELLENT band: surface area 0.151%, mean distance 0.0032 mm, Hausdorff 0.0098 mm, bounding box ≤ 0.005 mm/axis, centroid distance 0.0024 mm. All seven cross-section slices passed under 0.283% perimeter error (worst slice at Y = 17.533, the narrow neck just before the globe terminates — that region resolves to a thin cross-section so a small absolute deviation produces a relatively larger percentage, but at 0.283% it is still well inside the 🟢 band).
- **~5 min** cross-checking the result against the 1mm-post numbers to confirm the difference is just the globe diameter and not a regression somewhere. Hausdorff 0.0098 mm (vs 0.0092 on the 1mm-post), mean 0.0032 mm (vs 0.0029), surface area error 0.151% (vs 0.14%) — all within a few µm of each other, which is what you'd expect from two parts built with the same script template against two correctly-extracted source CSVs. The slightly looser numbers on the 2mm-post mostly reflect that this part has more surface area to cover (414 mm² vs 405 mm²) at the same FDM tessellation density, not any difference in the underlying parametric reconstruction.

Two notes worth recording for future parts:

- **A cleaner source CSV genuinely saves time.** The 1mm-post took 1 h 15 min and ~45 min of that was the spline parser + the bridge insertion + the rounded-key precision-leak debug. The 2mm-post had neither feature, ran on the first dry pass, and finished in 1 hour even though it has *more* primitives in S1. The lesson here is that the bulk of the template's per-part time is spent on CSV-extraction defects, not on geometry complexity. Investing in a tighter Fusion add-in or a more aggressive `0_preprocess_csvs.py` would pay off across the remaining 18 parts faster than micro-optimising the build123d side.
- **Strip what you don't need.** It is tempting to keep the bridge-insertion + spline-overflow machinery in every globe-post script "just in case future variants need it" — but doing so makes the script harder to read, slower to dry-run, and (worst) hides which features the part actually exercises. Each part script should declare its primitive set in a single `EXPECTED_*` block near G1 and carry only the parsers / repairers that block requires. The 2mm-post script's G1 section is ~80 lines vs the 1mm-post's ~200, and you can see the geometry rules at a glance.

### A note on `zaphod_bot_mec_fbcoup_5x15mm_presentation_post` — why it took 30 min

The 5x15mm_presentation_post is the fourth part and was the fastest build of the project at the time it landed. Same 4-guideline structure as the previous three globe-family parts (G1 = S1 profile, G2 = S2 axis line, G4 = surface-revolve, G3 = final export), but the source CSV exercises two new branches of the template that earlier parts never reached: the profile is a single **CLOSED loop** (12 primitives — 8 Lines + 4 three-point arcs — every endpoint with degree 2, zero free endpoints) and the sketch plane is **Z-constant** (`Z = 4.582`) rather than the X-constant planes the previous three parts all used. Both are first-of-their-kind in the repo. The 30 min was split roughly as:

- **~5 min** dropping the existing 2mm-post script into the new folder, swapping CSV paths via `BASE_DIR`, swapping the expected-primitive counts (`EXPECTED_LINES = 8`, `EXPECTED_ARCS = 4`), and updating the header docstring to describe the new geometry. The plane-detection branch for Z-constant sketches was already present and tested in the PICSY companion repo, so no detector change was needed — the helper infrastructure (CSV reading, plane detection, axis-parameterised `world_vec_axis`/`in_plane_uv`, `parse_line_segments`, `parse_three_point_arcs`, the checkpoint scaffold, the G4 OCCT revolve path with per-edge and `build123d.revolve()` fallbacks) all came across unchanged.
- **~10 min** adding the closed-loop walker variant. The existing `walk_open_chain` requires exactly 2 free endpoints; this part has 0, so a parallel `walk_closed_chain` was added that starts from `edges_named[0]` as authored, follows unused neighbours at each endpoint key, and validates the walk returns to the starting key after consuming every primitive. Same `_kep` 0.01 mm adjacency rounding, same unrounded-coordinates-for-edge-construction discipline. After the walk, the script also asserts `Wire.is_closed` — if the walker reports success but build123d still flags the wire as open, that surfaces a sub-tolerance endpoint mismatch immediately rather than producing a silently-open wire downstream. The Step-1 / Step-N free-endpoint ownership check from the open-chain walker is dropped — meaningless on a closed loop.
- **~10 min** on the first dry run and the G4 / comparison run. CSV freshness banner printed the two CSVs at 727 B and 95 B. Plane detector landed on `Z = 4.582` for S1 and `X = 4.397` for S2 (the S2 line is degenerate enough that both X and Z are constant — the detector picks X first; the lifted world-space axis line is identical either way). Parser counted 8 lines + 4 arcs in S1 and 1 line in S2 — passed the `EXPECTED_*` guard. Closed-loop walker visited all 12 edges first try, returned to the start key, and `Wire.is_closed` confirmed True. G4 produced 12 faces via `BRepPrimAPI_MakeRevol(wire, …)` in a single call at total area ~524.5 mm². `.clean()` ran without warning, STL+STEP exports landed cleanly.
- **~5 min** on the comparison run. Every aggregate metric landed in 🟢 EXCELLENT and this part posted the tightest aggregate numbers of the project at the time: surface area 524.5266 mm² vs reference 524.4362 mm² (**0.017%** error — an order of magnitude better than the three globe-family parts' 0.14–0.18%), Hausdorff 0.0144 mm, mean distance 0.0031 mm, bounding box ≤ 0.0032 mm/axis, centroid distance 0.0026 mm. All seven cross-section slices ≤ 0.178% perimeter error (worst at Y = 13.0). Both meshes register as watertight closed solids in trimesh — a closed wire revolved 360° about an external axis produces a torus-topology shell that comes out watertight when tessellated, even though the build123d-side intermediate is a face collection rather than a `Solid` object. The comparison script's surface-vs-surface metrics (area, Hausdorff, slices) apply unchanged; the closed-mesh status is incidental.

Three notes worth recording for future parts:

- **A second walker variant is a 10-minute change once the first one is in place.** The open-chain walker did the heavy lifting (fuzzy adjacency, unrounded-coordinate preservation, oriented-edge closures, reversible primitive build functions); the closed-loop variant reuses every piece of that and only differs in the start condition (any edge as start, vs free-endpoint as start) and termination condition (return-to-start, vs reach-other-free-end). The next time the repo needs another walker variant — say, multi-loop disjoint chains for trimmed faces — it should be in the same shape: share `_kep`, share the closure-based oriented-edge builders, share the unrounded-coords discipline, and only specialise the start/terminate logic.
- **The plane auto-detector being axis-agnostic paid off here.** Every prior part in this repo lived on an X-constant plane, so the Y- and Z- branches of `detect_sketch_plane` were dead code paths for the first three runs. They turned out to be correct anyway because the detector was written generically (iterate X → Y → Z, first one that's constant within `EPSILON_MM` wins) rather than special-cased per part. Worth noting because the temptation when writing the first few parts is to hardcode the axis — every minute saved on the first part by hardcoding would have cost ten minutes on this fourth part. The same logic should apply to any other axis-conditional code added in future parts (revolve axis direction, mirror plane selection, etc.).
- **Watertight comparison output is incidental, not engineered.** The build123d intermediate is a `Compound` of `Face` objects, not a `Solid`. The fact that the exported STL happens to register as watertight in trimesh is a property of the *tessellated mesh*, not of the build123d topology — a closed wire revolved about an external axis produces a shell with no boundary edges, which tessellates into a closed-mesh STL even though the build side never asks for a solid. For comparison purposes this means the existing surface-vs-surface metrics (area, Hausdorff, slices) apply unchanged. If a future part needs *actual* volume metrics (computed from the build123d topology rather than the post-tessellation mesh) the build script will need to construct a `Solid` explicitly — for example by patching the closed profile into a `Face` and using `revolve(face, axis, 360°)` (the high-level wrapper does work on a Face — only on bare Wires does it silently degenerate).

### A note on `zaphod_bot_mec_fbcoup_5x25mm_presentation_post` — why it took 20 min

The 5x25mm_presentation_post is the fifth part and is the **fastest build of the project so far**. It is the longer sibling of the 5x15mm_presentation_post — 30 mm long along Y instead of 20 mm — built from a **structurally identical source sketch**: same closed-loop profile of 8 Lines + 4 three-point arcs = 12 primitives on the same Z = 4.582 plane, same revolve axis at X = 4.397 on the same plane outside the profile's X-range, same torus-topology shell. The only differences in the source CSVs are (a) the profile's Y-range now spans 0.0..30.0 instead of 0.0..20.0 (the post is 10 mm longer), and (b) the S2 axis-line endpoints move proportionally from `(4.397, 20.0)→(4.397, 16.3)` to `(4.397, 30.0)→(4.397, 27.038)`. The 20 min was split roughly as:

- **~5 min** dropping the existing 5x15mm_presentation_post script into the new folder, swapping the CSV paths via `BASE_DIR`, and updating the header docstring to describe the new Y-extent and S2 axis-line endpoints. The expected-primitive counts (`EXPECTED_LINES = 8`, `EXPECTED_ARCS = 4`) carry across unchanged because S1's topology is bit-for-bit identical to the 5x15mm sibling — same primitive count, same connectivity. The entire helper infrastructure — CSV reading, plane detection, `world_vec_axis`/`in_plane_uv`, `parse_line_segments`, `parse_three_point_arcs`, `walk_closed_chain`, the checkpoint scaffold, the G4 OCCT revolve path — came across without a single character changed.
- **~3 min** on a pre-flight connectivity check before running the build script: tabulating the 12 primitive endpoints by `_kep` rounded key showed exactly 12 distinct keys, each shared by exactly 2 primitives, zero free endpoints — the same degree-2 closed-loop signature as the 5x15mm variant. Confirmed S1 was a clean closed loop ready for the closed-loop walker.
- **~7 min** on the first dry run and the G4 / comparison run. CSV freshness banner printed the two CSVs at 731 B and 97 B (vs 727 B and 95 B on the 5x15mm — the 4-byte and 2-byte deltas are exactly the longer Y values written into S1 and S2). Plane detector landed on `Z = 4.582` for S1 and `X = 4.397` for S2 — identical to the 5x15mm sibling. Parser counted 8 lines + 4 arcs in S1 and 1 line in S2 — passed the `EXPECTED_*` guard. Closed-loop walker visited all 12 edges first try, returned to the start key, and `Wire.is_closed` confirmed True. G4 produced 12 faces via `BRepPrimAPI_MakeRevol(wire, …)` in a single call at total area ~773.3 mm² (vs ~524.5 mm² on the 5x15mm — the difference is exactly the extra ~10 mm of cylindrical revolved surface that the longer post adds in the middle of the body). `.clean()` ran without warning, STL+STEP exports landed cleanly.
- **~5 min** on the comparison run. Every aggregate metric landed in 🟢 EXCELLENT, and the part essentially ties the 5x15mm_presentation_post for tightest aggregate run of the project so far: surface area 773.2635 mm² vs reference 773.1088 mm² (**0.020%** error vs 0.017% on the 5x15mm — both an order of magnitude better than the three globe-family parts), Hausdorff 0.0151 mm (vs 0.0144), mean distance 0.0033 mm (vs 0.0031), bounding box ≤ 0.0032 mm/axis on every axis, centroid distance 0.0028 mm. All seven cross-section slices ≤ 0.151% perimeter error (worst at Y = 19.5 — a region that didn't exist on the shorter 5x15mm part because Y = 19.5 sits past that part's 20 mm Y-extent, so it's a new patch of geometry being measured against the upstream reference STL for the first time). One asymmetry vs the 5x15mm sibling: the upstream reference STL for the 5x25mm registers as watertight in trimesh, but the build123d-side STL comes out as `is_watertight = False` — the same closed-wire-revolved-about-external-axis topology that tessellated to a closed mesh on the 5x15mm run produces a tessellated mesh with a small open-edge gap on the 5x25mm run. The opening is small enough that no surface-comparison metric is affected (the surface-vs-surface metrics — area, Hausdorff, slices — don't care about mesh closure), and the build123d topology is identical between the two siblings, so the gap is a tessellation artefact rather than a geometry difference. Noting it for completeness but it doesn't change the verdict.

Three notes worth recording for future parts:

- **A near-pure parametric clone of a sibling is a 5-minute build.** The 5x25mm_presentation_post differs from the 5x15mm_presentation_post in two CSV constants (the profile's Y-extent and the S2 axis-line endpoints) and nothing else — no new primitive types, no new walker variant, no new plane axis, no new revolve fallback. The script edit was a docstring rewrite plus four numeric constants in the header comment; the executable body is bit-for-bit identical. The build-side budget for variant parts whose differences boil down to source-CSV constants should be ~10–20 minutes total, dominated by running the comparison script and writing the README entry, not by editing the build script.
- **When a sibling part adds Y-extent, the new cross-section slices test new ground.** The slice runner picks 7 Y-planes from each mesh's own bounds, so the 5x25mm slices land at Y = 1.5, 6.0, 10.5, 15.0, 19.5, 24.0, 28.5 — three of those (Y = 19.5, 24.0, 28.5) sit entirely past the 5x15mm sibling's 20 mm Y-extent. The 5x25mm slice run is therefore measuring upstream-STL geometry that the 5x15mm slice run never touched. The fact that those three new slices all came out in the ≤ 0.151% band is genuine confirmation that the revolve is faithful along the full length of the longer body, not just an artefact of the 5x15mm sibling having already validated the body shape.
- **Tessellation closure on revolved closed-wire shells is not stable across sibling runs.** Both the 5x15mm and 5x25mm presentation_posts are built by the exact same topology pipeline (`BRepPrimAPI_MakeRevol` on a closed `Wire`, `.clean()`, `export_stl(tolerance=5e-4)`) and both produce 12 faces with no boundary edges in the build123d-side B-rep. But the tessellated STL of the 5x15mm comes out watertight while the 5x25mm comes out with a small open edge — same code, same operation, slightly different mesh outcome. Watertightness of the *tessellated mesh* is incidental, not engineered, and shouldn't be treated as part of the acceptance criteria for a surface part. The actual surface-comparison metrics (area, Hausdorff, slices, centroid, bbox) all apply equally to both meshes and are what the ✅ Complete badge is based on.
...
The **cross-section slice comparison** has proven especially valuable on complex parts: it cuts both meshes at multiple Y-plane positions (auto-derived from mesh bounds) and compares the outline perimeter at each cut. Aggregate metrics like Hausdorff or mean-distance can mask a 1–2% feature-level discrepancy because the affected region is small compared to the total surface — the slice test bypasses this by comparing geometry at specific cross-sections. On the five parts built so far every slice has passed comfortably: ≤ 0.24% perimeter error on the 1mm-post, ≤ 0.283% on the 2mm-post, ≤ 0.178% on the 5x15mm presentation_post, ≤ 0.151% on the 5x25mm presentation_post (the best slice run yet, including the three new slices past Y = 20 mm that the 5x15mm sibling didn't cover), with the worst slice on each globe-post landing at the narrow neck just before the globe terminates (where a small absolute deviation resolves to a relatively larger percentage of a small perimeter).

For **solid parts** the original volume + symmetric-volume-difference comparison from the [Thor assembly project](https://github.com/avajones081196/Ava_AngelLM_Thor_Art1_build123d) is used. Note that "solid" here refers to how the part is *built* in build123d (a `Solid` object or a face that revolves/extrudes into one), not whether the exported STL happens to tessellate as a watertight closed mesh — surface parts built from closed profile wires (e.g. the presentation_posts) may *or may not* register as watertight in mesh tools depending on tessellation luck (the 5x15mm variant did, the 5x25mm variant did not), but either way they are still surface-built and use the surface comparison script.
...
The same layout is used for every part. Output filenames carry the active guideline range (e.g. `zaphod_bot_mec_fbcoup_3mm_globe_G_1_4.stl`) so checkpoint exports do not clobber each other.
