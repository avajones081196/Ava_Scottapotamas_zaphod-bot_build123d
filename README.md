...
The main script reads the cleaned CSVs and rebuilds the geometry as a series of numbered guidelines (G1, G2, G3, …). The guideline count scales with part complexity — for the 3mm_globe, 5x15mm_1mm_globe_post, 5x15mm_2mm_globe_post and 5x15mm_presentation_post G1–G4 is enough (G3 is always the final export step). For larger parts the count will grow into the dozens, following the same pattern proven on the PICSY companion repo (where individual parts run up to G92). The pattern is:
...
- **Surface-revolve / extrude / loft** — the part-specific geometric operation. For the four globe-family parts (`3mm_globe`, `5x15mm_1mm_globe_post`, `5x15mm_2mm_globe_post`, `5x15mm_presentation_post`) this is a 360° revolve of a profile wire about a construction-axis line. For future parts it may include any of the extrude / loft / boolean / trim patterns documented in the PICSY companion repo.
...
- **~5 min** on the comparison run. Every aggregate metric landed in 🟢 EXCELLENT and this part posted the tightest aggregate numbers of the project so far: surface area 524.5266 mm² vs reference 524.4362 mm² (**0.017%** error — an order of magnitude better than the three globe-family parts' 0.14–0.18%), Hausdorff 0.0144 mm, mean distance 0.0031 mm, bounding box ≤ 0.0032 mm/axis, centroid distance 0.0026 mm. All seven cross-section slices ≤ 0.178% perimeter error (worst at Y = 13.0), which is the best slice run of the four parts so far. Both meshes register as watertight closed solids in trimesh — a closed wire revolved 360° about an external axis produces a torus-topology shell that comes out watertight when tessellated, even though the build123d-side intermediate is a face collection rather than a `Solid` object. The comparison script's surface-vs-surface metrics (area, Hausdorff, slices) apply unchanged; the closed-mesh status is incidental.
